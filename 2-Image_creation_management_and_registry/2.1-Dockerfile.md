...
```
RUN apt-get --assume-yes install curl && apt-get --assume-yes install git && \
    apt-get --assume-yes install tcpdump
```
...
```
docker run --detach --entrypoint php --name phpinfo --rm mylibrary/test-image:latest \
    -f index.php -S localhost:8080
```
