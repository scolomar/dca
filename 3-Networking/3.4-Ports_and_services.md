...
```
docker service create --detach --entrypoint curl --name test --network my-overlay-network \
    --restart-condition none nginx:alpine webserver --head --silent
```
...
```
replica_ip=$( docker inspect $( docker service ps webserver --quiet | tail -1 )\
    | grep Addresses -A1 | tail -1 | cut -d'"' -f2 | cut -d/ -f1 )
docker service create --detach --entrypoint curl --name test-ip --network my-overlay-network \
    --restart-condition none nginx:alpine ${replica_ip} --head --silent
```
...
```
vip=$( docker service inspect webserver | grep VirtualIPs -A4 \
    | awk -F'"' /Addr/'{ print $4 }' | cut -d/ -f1 )
docker service create --detach --entrypoint curl --name test-vip --network my-overlay-network \
    --restart-condition none nginx:alpine ${vip} --head --silent
```
