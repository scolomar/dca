...
docker run --entrypoint /usr/local/bin/python --network none --read-only \
    --user nobody:nogroup --volume ${PWD}/data/words.txt:/data/words.txt:ro \
    --volume ${PWD}/src/anagrams.py:/data/anagrams.py:ro --workdir /data/ \
    python anagrams.py
```
