...
docker run --detach --entrypoint php --name phpinfo --publish 8080 --restart always \
    --user nobody --volume /tmp/index.php:/app/index.php:ro --workdir /app/ \
...
docker service create --config source=index.php,target=/app/index.php,mode=0400,uid=65534 \
    --entrypoint php --mode replicated --name phpinfo --publish 8080 --read-only --replicas 2 \
    --restart-condition any --user nobody --workdir /app/ \
    php -f index.php -S 0.0.0.0:8080
```
...
```
docker service create --config source=index.php,target=/app/index.php,mode=0400,uid=nobody \
    --detach --entrypoint php --mode replicated --name phpinfo-failure --publish 8080 \
    --read-only --replicas 2 --restart-condition any --user nobody --workdir /app/ \
    php -f index.php -S 0.0.0.0:8080
```
