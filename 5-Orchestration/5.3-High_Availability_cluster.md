...
docker service create --config source=index.php,target=/app/index.php,mode=0400,uid=65534 \
    --constraint node.role==worker --entrypoint php --mode replicated --name phpinfo \
    --publish 8080 --read-only --replicas 1 --restart-condition any --user nobody --workdir /app \
...
```
The swarm does not have a leader. It's possible that too few managers are online.
