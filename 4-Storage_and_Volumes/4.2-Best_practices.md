...
```
docker run --detach --name test-volume-readonly --read-only --tty \
    --volume myvolume:/mydata/:ro busybox
```
We can attach as many volumes as needed:
```
docker run --detach --name test-volumes --read-only --tty --volume vol1:/data1/:rw \
    --volume vol2:/data2/:rw --volume vol3:/data3/:ro busybox
```
